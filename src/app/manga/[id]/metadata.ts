...
export async function generateMetadata(
  ...
  const description = manga.description === 'Updating'
    ? `Read ${manga.title} manga online for free at Manga Clan. ${altTitlesText}A ${manga.genres.join(', ')} manga by ${manga.authors.join(', ')}. ${manga.chapterList.length} chapters available.`
    : `${manga.description} ${altTitlesText}Read ${manga.title} manga online for free at Manga Clan.`;

  // Get latest chapter number and title
  const latestChapter = manga.chapterList[manga.chapterList.length - 1];
  const title = `${manga.title} - Read Online Free | Chapter 1-${latestChapter.id} | Manga Clan`;
  ...
  return {
    ...
    openGraph: {
      title: `${manga.title} - Read Free Online | Manga Clan`,
      description: description,
      type: 'article',
      images: [
        {
